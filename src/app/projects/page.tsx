  ...
  return (
    <main className="bg-texture  w-full h-full bg-primary bg-[50%,50%] min-h-screen">
      <p className="flex items-center px-6 pt-24 lg:px-16 lg:pt-32 lg:pb-8 pb-4">
        <Luna />
        <span className="text-base leading-4 italic">PROJECTS</span>
      </p>
      ...
      <div className="lg:p-20 px-6 py-12 grid lg:grid-cols-2 items-center justify-center lg:gap-x-10 gap-x-7 gap-y-12 lg:gap-y-16 mx-auto">
        {items.map((item) => (
          <div
            key={item.id}
          ...
          >
            ...
            <p className="flex gap-2 ">
              <span className="text-[.75rem] leading-4 uppercase font-medium">
                {item.title}
              </span>
              <small className=" text-primary-light leading-4">•</small>
              <span className="text-[.75rem] leading-4 text-primary-light font-medium">
                {item.year}
              </span>
            </p>
          </div>
